...
export class CustomToolTipsService  {
  ...
  <div class="Tool-tips-header-page top-bg-color">
    ...
    <span class="Tool-tips-footer-page" id="Tool_tips_footer_page"></span>
  </div>
...
`;
  ...
  ShowToolTips($event: any, message: any, title: any) {
    ...
    $(`.Tool-tips-middle-page`).html(CREATE_P_TAG);
      var H3_TAG:any = [];
      var CLASS_NAME:any = 'custom-popup-button';
      ...
      this.renderer.listen(H3_TAG, 'click', (event) => {
        ...
        $('#Tool-tips-custom-popup').hide().css('display', 'none');
      });
  ...
  getSessionLogin() {
    ...
    if (this.cookieService.getCookie('LoginDepotCode')!='' && this.cookieService.getCookie('LoginDepotCode')!=undefined) {
      var DUMP_DATA=JSON.parse(this.cookieService.deocde(this.cookieService.getCookie('LoginUser')));
      User_Login={
  ...
  OutSideCOntroller($buttonEvent: any, MAIN_CLASS_POPUP: any, CLOSE_DIV: any) {
    ...
    $(document).click(function (e: any) {
      if ('Tool_tips_footer_page' != $(e.target).attr('id') && 'navbarDropdown' != $(e.target).attr('id')) {
        $(CLOSE_DIV).hide();
      }
    });
  }
